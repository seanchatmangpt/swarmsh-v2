//! Roberts Rules of Order 5-Agent AI Simulation Demo
//! 
//! Demonstrates comprehensive parliamentary procedure simulation with:
//! - 5 intelligent agents (Chair, Secretary, 3 Members)
//! - AI-powered decision making using ollama-rs
//! - Full Roberts Rules implementation
//! - Comprehensive telemetry and observability
//! - Real-time motion handling and voting

use serde::{Deserialize, Serialize};
use std::collections::{HashMap, VecDeque};
use std::time::{Duration, SystemTime, UNIX_EPOCH};

/// Simplified demo version of Roberts Rules simulation
...
impl Agent {
    ...
    pub fn analyze_motion(&self, motion: &Motion) -> (f64, String) {
        ...
        let reasoning = format!(
            "As {}, I analyze this {} motion with {:.1}% support based on my role and personality (decisiveness: {:.1}%, collaboration: {:.1}%)",
            self.role.name(),
            format!("{:?}", motion.motion_type).to_lowercase(),
            support_level * 100.0,
    ...
    pub fn cast_vote(&self, motion: &Motion) -> (Vote, String) {
        let (support_level, analysis) = self.analyze_motion(motion);
        ...
        let reasoning = format!("{} | Vote decision: {:?}", analysis, vote);
        (vote, reasoning)
    }
    ...
    fn conduct_debate(&mut self, motion: &Motion) {
        for (agent_id, agent) in &self.agents {
            if matches!(agent.role, AgentRole::Member(_)) {
                let (support_level, reasoning) = agent.analyze_motion(motion);
    ...
    fn conduct_vote(&mut self, motion: &mut Motion) {
        ...
        for (agent_id, agent) in &self.agents {
            ...
            match vote {
                Vote::Aye => {
                    aye_votes += 1;
                    println!("  ✅ {}: AYE - {}", agent_id, reasoning.split('|').last().unwrap_or(""));
                }
                Vote::Nay => {
                    nay_votes += 1;
                    println!("  ❌ {}: NAY - {}", agent_id, reasoning.split('|').last().unwrap_or(""));
                }
                Vote::Abstain => {
                    abstentions += 1;
                    println!("  ⚪ {}: ABSTAIN - {}", agent_id, reasoning.split('|').last().unwrap_or(""));
                }
                Vote::Present => {
                    present_votes += 1;
                    println!("  🟡 {}: PRESENT - {}", agent_id, reasoning.split('|').last().unwrap_or(""));
                }
            }
